package com.reece.myrpc.version6.register.impl;

import com.reece.myrpc.version6.loadbalance.LoadBalance;
import com.reece.myrpc.version6.loadbalance.impl.RandomLoadBalance;
...
public class ZKRegisterCenter implements RegisterCenter {
    ...
    public ZKRegisterCenter(LoadBalance loadBalance) {
    ...
    public InetSocketAddress getServiceAddressByServiceName(String serviceName) {
        ...
        try {
            List<String> addressList = client.getChildren().forPath(PREFIX + serviceName);
            //这里先使用第一个  后边使用负载均衡策略
            String serviceAddress = loadBalance.getAddress(addressList);
            System.out.println("使用的serviceAddress:" + serviceAddress);
